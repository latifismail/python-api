...
from ..conf import database, models, schemas
...
def get_post(id, db: Session = Depends(database.get_db)):
...
def create_post(payload: schemas.PostCreate, db: Session = Depends(database.get_db)):
...
def update_post(id, payload: schemas.PostUpdate, db: Session = Depends(database.get_db)):
    post = db.query(models.Post).filter(models.Post.id == id)
    if post.first() == None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Post not found")
